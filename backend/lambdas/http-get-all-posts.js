...
exports.handler = async (event) => {
    ...
    let comments = timeReactionComments.data.filter(k=> k.comments != undefined).map(j=>(j.comments.data[0].message))

    let objs = await Promise.all(ids.map(async(i)=>{
        return axios.get(`https://graph.facebook.com/v13.0/${i.id}/insights/post_impressions?access_token=${pageToken}`)
    }))
    let page_impressions = objs.map(o=>o.data.data[0].values[0].value)
    let resp = {ids, page_impressions, comments}

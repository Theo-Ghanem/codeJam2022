...
const getIGPosts = (instaId, userToken) => {
  let allInstaPosts = (
    await axios.get(
...
exports.handler = async (event) => {
  ...
  const IG = getIGPosts(instaId, userToken);
  const FB = getFBPosts(pageToken);
  console.log(IG, FB);
  ...
  fbAvg = (igAvg / FBPosts.length) * 50 + 50;
